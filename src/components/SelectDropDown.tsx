...
const SelectDropDown = ({selected, setSelected} : ISelect) => {
  console.log(selected);
  const CategoriesCreator = categories.map((category) => (
    <ListboxOption
      key={category.id}
      value={category}
      className={"group relative cursor-default select-none py-2 pl-3 pr-9 text-gray-900 data-[focus]:bg-indigo-600 data-[focus]:text-white"}>
      <div className={"flex items-center"}>
        <img alt={category.name} src={category.imageURL} className={"h-5 w-5 flex-shrink-0 rounded-full"}/>
      ...
      <div className={"relative mt-2"}>
        <ListboxButton className={"relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6"}>
          <span className={"flex items-center"}>
            <img src={selected.imageURL} className={"h-5 w-5 flex-shrink-0 rounded-full"}/>
